...
void get_ymd_hms_ut( time_t ut_time, int& year, int& month, int& day, int& hour, int& minute, int& sec );
...
public :
   ...
   InputMapping() 
   : input(-1), antenna(-1), pol('U'), delta(0), flag(0)
   {};
...
{
   sInputIndex()
   : pol_x_input_index(-1), pol_y_input_index(-1)
   {}
   
   int pol_x_input_index;
   int pol_y_input_index;
};
...
//		MWAInput &InputRW(const size_t index) { return _inputs[index]; }
		...
		size_t CentreSubbandNumber() const;
                ...
                // inputs / antennas / pols :
                bool IsAntennaFlagged( int antenna );
                InputMapping* getInputInfo( int antenna , int pol );
                int getInput( int antenna , int pol );
                int GetAntennaIndex( int input , int& pol );
                int GetBaselineIndex( int input1, int input2 );
                std::vector<InputMapping> m_Inputs;		
                std::vector<sInputIndex>  m_AntToInputMapping;
		int read_mapping_file( const char* filename="instr_config.txt" );
		
		// Correlator matrix :
		std::vector< std::vector<int> > m_CorrelatorMatrix;
