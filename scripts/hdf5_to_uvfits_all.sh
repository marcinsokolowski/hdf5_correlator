...
if [[ -s /opt/aavs/config/station.yml ]]; then
   station_name=`awk -v station_section=0 '{if(index($1,":")>0 && NF==1){if(index($1,"station")>0 ){station_section=1;}else{station_section=0;}}if(station_section>0){if($1=="name:"){station_name=$2;gsub("\"","",station_name);gsub("\r","",station_name);print tolower(station_name);}}}' /opt/aavs/config/station.yml`
...
#                 rm -f $mypipe
       ...
       if [[ -d ${aavs_calibration_path} ]]; then
           ...
           # TEMPORARY unitl Randall commits config files 
           # aavs_calibration_path=~/aavs-calibration/
           echo "${aavs_calibration_path}/Lfile2uvfits_eda.sh -i ${inttime} -n ${n_integrations_per_uvfits} ${radec_string} -N 512 -C ${n_chan} -f ${freq_channel} -s ${station_name} ${lfile_base_corr}"
