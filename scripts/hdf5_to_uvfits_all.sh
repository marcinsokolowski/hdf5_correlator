...
#    exit
       ...
       if [[ $bin2lfiles -gt 0 ]]; then
          ...
          if [[ -d ${aavs_calibration_path} ]]; then       
             ...
             echo "${aavs_calibration_path}/Lfile2uvfits_eda.sh -i ${inttime} -n ${n_integrations_per_uvfits} ${radec_string} -N 512 -C ${n_chan} -f ${freq_channel} -F ${lfile_base}"
             ${aavs_calibration_path}/Lfile2uvfits_eda.sh -i ${inttime} -n ${n_integrations_per_uvfits} ${radec_string} -N 512 -C ${n_chan} -f ${freq_channel} -F ${lfile_base}
          else
