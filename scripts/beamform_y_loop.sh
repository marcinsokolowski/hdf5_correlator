...
tag="calsol20200224_ch126_Gero_Y"
if [[ -n "$3" && "$3" != "-" ]]; then
   tag="$3"
fi
...
do
   ...
   while [[ $az -le 360 ]];
   do
      echo "beamform_all.sh \"-C 1 -X $delays -K ($az,$elev) -E antenna_locations_eda2.txt -e Geraldton_az${az}el${elev}deg_timeseries_Y -p 1 -H $freq_mhz\" 20200225_24hours_eda2_256ant_ch126_GeroBeam_az${az}el${elev}deg_Y.txt - hdf5_list > az${az}el${elev}deg_y.out 2>&1"
      beamform_all.sh "-C 1 -X $delays -K ($az,$elev) -E antenna_locations_eda2.txt -e Geraldton_az${az}el${elev}deg_timeseries_Y -p 1 -H $freq_mhz" 20200225_24hours_eda2_256ant_ch126_GeroBeam_az${az}el${elev}deg_Y.txt - hdf5_list > az${az}el${elev}deg_y.out 2>&1
