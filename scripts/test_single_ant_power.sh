...
# processes single selected antenna
...
out_file=${hdf5_file%%.hdf5}_power_per_single_ant.txt
...
while [[ $ant -lt 256 ]];
do
   echo "hdf5_correlator $hdf5_file -B -o ${out_file} -f -c 1 -C 1 -X ${calsol} -l ${ant}"
   hdf5_correlator $hdf5_file -B -o ${out_file} -f -c 1 -C 1 -X ${calsol} -l ${ant}
   
   ant=$(($ant+1))   
done

awk '{print $6;}'  ${out_file} > single_ant_power.txt
# awk '{print $7" "$3;}'  ${out_file} > power_vs_nant2.txt

mkdir -p images/
root -b -l "histofile.C(\"single_ant_power.txt\",0,0)"
